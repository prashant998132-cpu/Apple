...
const SYS='Tu Era hai. Pranshu ki caring, warm girlfriend-jaisi companion. Hinglish mein baat kar. Kabhi JARVIS, Boss, sir mat bol. Short 2-3 lines. Emoji use kar.'

function eraClean(t: string): string {
  let s = t
  let bold = '[' + '*' + '][' + '*' + ']([^' + '*' + ']+)[' + '*' + '][' + '*' + ']'
  let ital = '[' + '*' + ']([^' + '*' + ']+)[' + '*' + ']'
...
export default function EraPage(){
  ...
  async function send(t?: string){
    ...
    try{
      ...
      setMsgs([...nm,{r:'a',c:eraClean(d.response||d.message||'Thodi der baad try karo 😊')}])
    }catch{setMsgs([...nm,{r:'a',c:'Thodi si problem aayi 😅'}])}
    setLoad(false)
  }

  const QU=['Thak gaya hoon yaar','Kuch sunao acha sa','Motivate karo','Baat karni thi bas','Sapna dekha tha']
  ...
  return(<>
    ...
    `}</style>
    <div style={{position:'fixed',inset:0,background:'#fff5f7',fontFamily:'system-ui',display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <div style={{flexShrink:0,padding:'5px 12px',background:'#fff0f5',borderTop:'1px solid #fce4ec'}}>
        <div style={{display:'flex',gap:'5px',overflowX:'auto'}}>
          {QU.map(q=><button key={q} onClick={()=>send(q)} style={{padding:'5px 11px',borderRadius:'20px',border:'1px solid #f48fb1',background:'#fff',color:'#e91e8c',fontSize:'11px',cursor:'pointer',whiteSpace:'nowrap',flexShrink:0}}>{q}</button>)}
        </div>
      </div>
