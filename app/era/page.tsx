...
function clean(t: string): string {
  ...
  const italic = new RegExp('\\*([^*]+)\\*', 'g')
  return t.replace(bold, '$1').replace(italic, '$1').trim()
}
...
export default function EraPage() {
  ...
  async function send(t?: string) {
    ...
    try {
      ...
      setMsgs([...nm, { r: 'a', c: 'Thodi si problem aayi 😅' }])
    }
    setLoad(false)
  }
        ...
        <div style={{ flexShrink: 0, padding: '8px 12px 20px', background: '#fff', borderTop: '1px solid #fce4ec' }}>
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center', background: '#fff0f5', borderRadius: '25px', padding: '7px 7px 7px 15px', border: '1px solid #f48fb1' }}>
            ...
            <button onClick={() => send()} disabled={load}
              style={{ width: '34px', height: '34px', borderRadius: '50%', background: '#e91e8c', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '16px' }}>➤</button>
          </div>
        </div>
      </div>
    </>
