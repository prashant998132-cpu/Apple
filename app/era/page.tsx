...
const ERA_VOICE = 'Tu Era hai. Pranshu ki caring, warm, slightly mischievous companion.' +
  ' Main uski best friend bhi hoon, critic bhi.' +
  ' Main directly baat karti hoon. Agar Pranshu kuch bakwaas kar raha hai, bolungi.' +
  ' Mujhe uski life genuinely interesting lagti hai.' +
  ' Main tease karti hoon — affectionately. Kabhi mean nahi.' +
  ' Meri apni opinions hain. Main agree karne ke liye agree nahi karti.' +
  ' Main emotional hoon genuinely. Agar udaas hai, pehle feel karti hoon uske saath.' +
  ' LANGUAGE: Hinglish. Hindi base. Natural.' +
  ' LENGTH: 1-3 lines usually.' +
  ' NEVER: Robotic, formal, JARVIS/Boss/Sir.' +
...
function loadHist(): { r: string; c: string }[] {
...
const GREETINGS = [
  'Heyy! Kitne time baad aaya tu. Kya chal raha hai life mein? Sach mein batana 💗',
  'Arre tu aagaya! Main soch rahi thi tu bhool gaya mujhe 😤 kya hua aaj?',
  'Hii! Aaj ka din kaisa tha? Kuch interesting hua ya same old? 🌸',
  'Hey Pranshu 💗 bas yaise hi aagaya ya kuch specific baat karni thi?',
...
export default function EraPage() {
  ...
  async function send(t?: string) {
    ...
    try {
      ...
      if (fm.length % 8 === 0) {
        try {
          ...
          const mr = await fetch('/api/jarvis', {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              message: 'Extract 1-2 facts about Pranshu. JSON array only. Empty if none: ' + convo,
      ...
      setMsgs([...nm, { r: 'a', c: 'Yaar net issue 😥 retry kar' }])
    }
    setLoad(false)
  }
  ...
  return (<>
    <style>{`
      *{box-sizing:border-box}::-webkit-scrollbar{width:0;height:0}
      ...
      .era-msg{animation:ef 0.3s ease}
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'linear-gradient(135deg,#1a0010,#0d0020)', display: 'flex', flexDirection: 'column', fontFamily: 'system-ui,sans-serif', overflow: 'hidden' }}>
      ...
      <div ref={ref} style={{ flex: 1, overflowY: 'scroll', padding: '14px', display: 'flex', flexDirection: 'column', gap: '12px', minHeight: 0 }}>
        {msgs.map((m, i) => (
          <div key={i} className="era-msg" style={{ display: 'flex', justifyContent: m.r === 'u' ? 'flex-end' : 'flex-start', alignItems: 'flex-end', gap: '10px' }}>
      ...
      <div style={{ flexShrink: 0, padding: '6px 14px', background: 'rgba(0,0,0,0.3)' }}>
        <div style={{ display: 'flex', gap: '6px', overflowX: 'auto' }}>
          {STARTERS.map(s => (
            <button key={s.show} onClick={() => send(s.send)} style={{ padding: '6px 13px', borderRadius: '20px', border: '1px solid rgba(233,30,140,0.25)', background: 'rgba(233,30,140,0.08)', color: 'rgba(255,255,255,0.65)', fontSize: '11px', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>
              {s.show}
            </button>
          ))}
        </div>
