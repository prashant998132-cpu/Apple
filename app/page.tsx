    ...
    <div style={{background:'#0a1628',border:'1px solid #1a3a5a',borderRadius:'16px',padding:'12px',maxWidth:'240px'}}>
      ...
      {rows.map((row,ri)=>(
        <div key={ri} style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'5px',marginBottom:'5px'}}>
          {row.map(k=>(
            <button key={k} onClick={()=>tap(k)} style={{padding:'11px 4px',borderRadius:'8px',border:'none',cursor:'pointer',fontSize:'14px',fontWeight:600,
              background:k==='='?'#00e5ff22':k==='C'?'#ff525222':['DIV','MUL','-','+'].includes(k)?'#1a3a5a':'#111d2e',
              color:k==='='?'#00e5ff':k==='C'?'#ff5252':'#e0f0ff'}}>{sym[k]||k}</button>
          ))}
        </div>
      ))}
    </div>
...
export default function Home() {
  ...
  async function speak(text: string) {
    if(!tts)return
    try{const r=await fetch('/api/tts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:text.substring(0,300)})});if(r.ok){const b=await r.blob();new Audio(URL.createObjectURL(b)).play()}}catch{}
  }
        ...
        <div style={{display:'flex',gap:'6px',overflowX:'auto'}}>
          {QUICK.map(q=>(
            <button key={q.l} onClick={()=>{
              ...
              if(q.m.endsWith(' '))setInp(q.m);else send(q.m)
            }} style={{padding:'6px 12px',borderRadius:'18px',border:'1px solid '+bc,background:card,color:tc,fontSize:'11px',cursor:'pointer',whiteSpace:'nowrap',flexShrink:0}}>{q.l}</button>
          ))}
        </div>
      </div>
