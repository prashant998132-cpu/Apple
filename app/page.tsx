...
function CalcWidget() {
  ...
  function tap(k: string) {
    ...
    if (k === '=') {
      try {
        const safe = expr.replace(/div/g, '/').replace(/mul/g, '*')
        const r = Function('"use strict"; return (' + safe + ')')()
        setRes(String(r)); setExpr(String(r))
      } catch { setRes('Err') }
      return
    }
    const sym = k === 'div' ? '/' : k === 'mul' ? '*' : k
    setExpr(e => e + sym)
    try {
      const safe = (expr + sym).replace(/div/g, '/').replace(/mul/g, '*')
  ...
  const labels: Record<string,string> = { div: '÷', mul: '×', DEL: '←' }
        ...
        <div key={ri} style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr',gap:'5px',marginBottom:'5px'}}>
          {row.map(k => (
            <button key={k} onClick={() => tap(k)} style={{
              padding:'11px 4px', borderRadius:'8px', border:'none', cursor:'pointer',
              fontSize:'14px', fontWeight:600,
              background: k==='=' ? '#00e5ff22' : k==='C' ? '#ff525222' : ['div','mul','-','+'].includes(k) ? '#1a3a5a' : '#111d2e',
              color: k==='=' ? '#00e5ff' : k==='C' ? '#ff5252' : '#e0f0ff'
            }}>{labels[k] || k}</button>
          ))}
        </div>
      ))}
...
function QrWidget({ text }: { text: string }) {
  ...
  return (
    <div style={{background:'#0a1628',border:'1px solid #1a3a5a',borderRadius:'16px',padding:'12px',maxWidth:'220px'}}>
      ...
      <input value={inp} onChange={e => setInp(e.target.value)} onKeyDown={e => e.key==='Enter' && setUrl(getQrUrl(inp))}
        placeholder="Text ya URL..." style={{width:'100%',background:'#030a14',border:'1px solid #1a3a5a',borderRadius:'8px',color:'#e0f0ff',padding:'6px 10px',fontSize:'12px',outline:'none',marginBottom:'8px'}}/>
      <button onClick={() => setUrl(getQrUrl(inp))} style={{width:'100%',background:'#00e5ff22',border:'1px solid #00e5ff',borderRadius:'8px',color:'#00e5ff',padding:'6px',cursor:'pointer',fontSize:'12px',marginBottom:'8px'}}>Generate</button>
...
const QUICK = [
  ...
  {l:'Cricket',m:'cricket score'},
  {l:'Video',m:'video banao '},
]

export default function Home() {
  const [msgs, setMsgs] = useState<Msg[]>([
    { r: 'a', c: 'Hey Pranshu! Main JARVIS hoon. Kuch bhi poochho ya type karo. Seedha kaam hoga! ' }
  ])
  ...
  const send = useCallback(async (text?: string) => {
    ...
    if (msg.startsWith('/')) {
      ...
      if (cmd === 'password') { setMsgs(m => [...m, { r: 'u', c: msg }, { r: 'a', c: 'Password: ' + genPassword() }]); return }
    ...
    // Image gen — client side, instant
    if (/image banao|photo banao|picture banao|wallpaper banao|generate image|art banao/.test(m)) {
      const prompt = msg.replace(/image banao|photo banao|picture banao|wallpaper banao|generate image|art banao/gi, '').trim() || msg
      ...
      setMsgs([...nm, { r: 'a', c: prompt, imageUrl: url }])
    ...
    if (/video banao|clip banao/.test(m)) {
      ...
      setMsgs([...nm, { r: 'a', c: 'Video generate ho raha hai (30-60s)...', videoUrl: 'https://video.pollinations.ai/' + encodeURIComponent(prompt) }])
      setLoad(false); return
    }

    // Password
    if (/password banao|strong password/.test(m)) {
      setMsgs([...nm, { r: 'a', c: 'Password: ' + genPassword() + '\n\nCopy kar lo!' }])
      setLoad(false); return
    }
    ...
    try {
      const r = await fetch('/api/jarvis', {
        ...
        body: JSON.stringify({ message: msg, conversationHistory: nm.slice(-8).map(x => ({ r: x.r, c: x.c })) })
      })
      const d = await r.json()
  ...
  const bg = dark ? '#040e1a' : '#f0f4ff'
  const card = dark ? '#0a1628' : '#fff'
  const tc = dark ? '#e0f0ff' : '#1a2040'
  const bc = dark ? '#1a3a5a' : '#d0d8f0'
  ...
  return (
    <div style={{position:'fixed',inset:0,background:bg,display:'flex',flexDirection:'column',fontFamily:'system-ui,sans-serif',color:tc}}>
      ...
      <div style={{flexShrink:0,background:dark?'#030a14':'#fff',borderBottom:'1px solid '+bc,padding:'10px 14px',display:'flex',alignItems:'center',gap:'10px',zIndex:20}}>
        <button onClick={() => setMenu(v => !v)} style={{background:'none',border:'none',cursor:'pointer',fontSize:'20px',color:tc}}>{'='}</button>
        <div style={{flex:1}}>
          <div style={{fontWeight:800,fontSize:'15px',letterSpacing:'1px',color:'#00e5ff'}}>JARVIS</div>
          <div style={{fontSize:'9px',color:'#4fc3f7',letterSpacing:'2px'}}>AI ASSISTANT v10.41</div>
        </div>
        <button onClick={() => setTts(v => !v)} style={{background:'none',border:'none',cursor:'pointer',fontSize:'16px',opacity:tts?1:0.3}} title="TTS">{'[S]'}</button>
        <button onClick={() => setTheme(t => t==='dark'?'light':'dark')} style={{background:'none',border:'none',cursor:'pointer',fontSize:'16px',color:'#00e5ff'}}>
          {theme==='dark'?'[L]':'[D]'}
        </button>
      </div>
      ...
      {menu && (
        <div style={{position:'fixed',inset:0,zIndex:100,display:'flex'}}>
          <div style={{width:'220px',background:dark?'#040e1a':'#fff',borderRight:'1px solid '+bc,padding:'16px',display:'flex',flexDirection:'column',gap:'8px'}}>
            <div style={{fontWeight:700,fontSize:'14px',color:'#00e5ff',marginBottom:'8px'}}>JARVIS Menu</div>
            {[['JARVIS','/'],['LUNA /luna','/luna'],['Era /era','/era'],['Tools /tools','/tools']].map(([l,h]) => (
              <a key={h} href={h} style={{display:'block',padding:'10px 12px',borderRadius:'10px',background:card,border:'1px solid '+bc,color:tc,textDecoration:'none',fontSize:'13px'}}>{l}</a>
            ))}
            <div style={{borderTop:'1px solid '+bc,paddingTop:'8px',marginTop:'4px'}}>
              <div style={{fontSize:'11px',color:'#888',marginBottom:'6px'}}>Slash Commands:</div>
              {['/calc','/qr [text]','/password','/clear','/luna','/era'].map(c => (
                <div key={c} style={{fontSize:'11px',color:'#4fc3f7',padding:'2px 0'}}>{c}</div>
              ))}
            </div>
          </div>
      ...
      <div ref={ref} style={{flex:1,overflowY:'scroll',padding:'12px',display:'flex',flexDirection:'column',gap:'12px',minHeight:0}}>
        {msgs.map((m, i) => (
          <div key={i} style={{display:'flex',justifyContent:m.r==='u'?'flex-end':'flex-start',alignItems:'flex-end',gap:'8px'}}>
            {m.r==='a' && <div style={{width:'28px',height:'28px',borderRadius:'50%',background:'linear-gradient(135deg,#00e5ff,#8b5cf6)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'12px',flexShrink:0}}>J</div>}
            <div style={{maxWidth:'80%'}}>
              ...
              {!m.widget && !m.imageUrl && !m.videoUrl && m.c && (
                <div style={{padding:'10px 14px',borderRadius:m.r==='u'?'18px 18px 4px 18px':'18px 18px 18px 4px',background:m.r==='u'?'#00e5ff15':card,border:'1px solid '+(m.r==='u'?'#00e5ff33':bc),color:tc,fontSize:'13.5px',lineHeight:'1.65',whiteSpace:'pre-wrap'}}>
                  {m.c}
                  {m.r==='a' && tts && <button onClick={() => speak(m.c)} style={{display:'block',marginTop:'4px',background:'none',border:'none',color:'#4fc3f7',cursor:'pointer',fontSize:'10px'}}>[Suno]</button>}
                </div>
              )}
            </div>
          </div>
        ...
        {load && (
          <div style={{display:'flex',gap:'8px',alignItems:'flex-end'}}>
            <div style={{width:'28px',height:'28px',borderRadius:'50%',background:'linear-gradient(135deg,#00e5ff,#8b5cf6)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'12px'}}>J</div>
            <div style={{padding:'10px 14px',background:card,border:'1px solid '+bc,borderRadius:'18px 18px 18px 4px',display:'flex',gap:'4px',alignItems:'center'}}>
      ...
      <div style={{flexShrink:0,padding:'5px 12px',background:dark?'#030a14':'#f8faff',borderTop:'1px solid '+bc}}>
        <div style={{display:'flex',gap:'5px',overflowX:'auto'}}>
          {QUICK.map(q => (
            <button key={q.l} onClick={() => { if (q.m.endsWith(' ')) setInp(q.m); else send(q.m) }}
              style={{padding:'5px 11px',borderRadius:'16px',border:'1px solid '+bc,background:card,color:tc,fontSize:'11px',cursor:'pointer',whiteSpace:'nowrap',flexShrink:0}}>{q.l}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{flexShrink:0,padding:'8px 12px 20px',background:dark?'#030a14':'#fff',borderTop:'1px solid '+bc}}>
        <div style={{display:'flex',gap:'6px',alignItems:'center',background:card,borderRadius:'26px',padding:'6px 6px 6px 14px',border:'1.5px solid '+(inp?'#00e5ff':bc)}}>
          <label style={{cursor:'pointer',fontSize:'16px',flexShrink:0,opacity:0.6}} title="Photo">
            [P]<input type="file" accept="image/*" style={{display:'none'}} onChange={handlePhoto}/>
          </label>
          <input ref={inpRef} value={inp} onChange={e => setInp(e.target.value)} onKeyDown={e => e.key==='Enter' && !e.shiftKey && send()}
            placeholder={recording?'Listening...':'Kuch bhi poocho, /calc, /qr...'}
            style={{flex:1,border:'none',background:'transparent',outline:'none',fontSize:'14px',color:tc}}/>
          <button onClick={toggleVoice} style={{width:'34px',height:'34px',borderRadius:'50%',background:recording?'#ff525233':'transparent',border:recording?'2px solid #ff5252':'none',cursor:'pointer',fontSize:'14px',flexShrink:0}}>{recording?'[]':'[M]'}</button>
          <button onClick={() => send()} disabled={load||!inp.trim()} style={{width:'34px',height:'34px',borderRadius:'50%',background:inp.trim()?'linear-gradient(135deg,#00e5ff,#8b5cf6)':'#1a3a5a',border:'none',cursor:'pointer',fontSize:'14px',flexShrink:0,color:inp.trim()?'#000':'#888'}}>{'>'}</button>
        </div>
      </div>
      <style>{`@keyframes p{0%,60%,100%{transform:translateY(0)}30%{transform:translateY(-6px)}}::-webkit-scrollbar{width:0}`}</style>
    </div>
  )
}
