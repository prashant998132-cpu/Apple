...
export default function ChatPage() {
  ...
  const refreshLoc = useCallback(async () => {
    try {
      ...
      if (!loc) return;
      syncLocationToCloud('mayur', loc).catch(() => {});
      ...
      if (chatMode === 'deep') {
        try {
          ...
          const ds = await fetch('/api/tool-stream', {
            ...
            body: JSON.stringify({ message: enrichedText || text, userId: localStorage.getItem('jarvis_uid')||'user', chatId: chatId.current, chatMode: 'deep', history: cur.slice(-8), memoryPrompt: memPrompt }),
          })
          if (ds.ok && ds.body) {
