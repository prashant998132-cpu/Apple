...
export async function routeTools(message: string): Promise<ToolResult[]> {
  ...
  const toolMap: Record<string, () => Promise<ToolResult>> = {
    ...
    reddit:     () => getRedditFeed(message),
    sports:     () => getSportsNews(message),
    ...
    urban:      () => getUrbanDictionary(message),
    ghprofile:  () => getGitHubProfile(message),
  }

  const toRun = intents.slice(0, 3).filter(i => toolMap[i])
